...
class WebDriverIO extends Helper {
  ...
  selectOption(select, option) {
    return findFields.call(this, select).then((res) => {
      ...
      return this.browser.unify(commands, {
        ...
        let normalizedValue, byValue;

        option.forEach((opt) => {
          normalizedValue = `[normalize-space(@value) = "${opt.trim() }"]`;
          byValue = `./option${normalizedValue}|./optgroup/option${normalizedValue}`;
  ...
  dragAndDrop(srcElement, destElement) {
    ...
    if (client.isMobile) {
      return client.element(withStrictLocator(srcElement)).then(function (strictSrcRes) {
        if (!strictSrcRes.value || strictSrcRes.value.length === 0) return truth(`elements of ${srcElement}`, 'to be seen').assert(false);
        let elem = strictSrcRes.value;
        return this.elementIdLocation(elem.ELEMENT).then(function (srcLocation) {
          if (!srcLocation.value || srcLocation.value.length === 0) {
            throw new Error(
              `Failed to receive (${srcElement}) location`);
          }
          return this.touchDown(srcLocation.value.x, srcLocation.value.y).then(function (touchDownRes) {
            if (touchDownRes.state !== 'success') throw new Error(`Failed to touch button down on (${srcElement})`);
            return client.element(withStrictLocator(destElement)).then(function (strictDestRes) {
              if (!strictDestRes.value || strictDestRes.value.length === 0) {
                return truth(`elements of ${destElement}`, 'to be seen')
                  .assert(false);
              }
              elem = strictDestRes.value;
              return this.elementIdLocation(elem.ELEMENT).then(function (destLocation) {
                if (!destLocation.value || destLocation.value.length === 0) {
                  throw new Error(
                    `Failed to receive (${destElement}) location`);
                }
                return this.touchMove(destLocation.value.x, destLocation.value.y).then(function (moveRes) {
                  if (moveRes.state !== 'success') throw new Error(`Failed to touch move to (${destElement})`);
                  return this.touchUp(destLocation.value.x, destLocation.value.y);
                });
              });
            });
          });
    ...
    return this2.moveCursorTo(withStrictLocator(srcElement)).then(function (moveRes) {
      if (moveRes.state !== 'success') throw new Error(`Unable to move cursor to (${srcElement})`);
      return this.buttonDown().then(function (buttonRes) {
        if (buttonRes.state !== 'success') throw new Error(`Failed to press button down on (${srcElement})`);
        return this2.moveCursorTo(withStrictLocator(destElement)).then(function (cursorRes) {
          if (cursorRes.state !== 'success') throw new Error(`Unable to move cursor to (${destElement})`);
...
function findFields(locator) {
  ...
  return this._locate(byText).then((elsByText) => {
    if (elsByText.value.length) return elsByText;
    let byName = `.//*[self::input | self::textarea | self::select][@name = ${literal}]`;
    return this._locate(byName).then((elsByName) => {
      if (elsByName.value.length) return elsByName;
...
function findCheckable(locator, locateFn) {
  ...
  return locateFn(byText).then(function (byTextEls) {
    if (byTextEls.value.length) return byTextEls;
    let byName = `.//input[@type = 'checkbox' or @type = 'radio'][@name = ${literal}]`;
    return locateFn(byName).then(function (byNameEls) {
      if (byNameEls.value.length) return byNameEls;
      return locateFn(locator); // by css or xpath
    });
  });
}
...
function prepareLocateFn(context) {
  ...
  return (l) => {
    ...
    return this._locate(context, true).then((res) => {
      ...
      el = res.value[0].ELEMENT;
      return this.browser.elementIdElements(el, l);
    });
  };
}
